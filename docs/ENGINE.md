...
- ▶️ **Sljedeći korak:** korisnik ponovno testira. **Otvoreni „drugi problem" (screenshot 16:50): STABILNOST rasporeda** — kad se doda jedan artikl, motor prepakira SVE od nule (reaktivni recompute) pa se raspored bitno pomakne. Cilj #4 iz 1.4. Pravi popravak = inkrementalno/stabilno pakiranje (zadrži postojeće, samo ubaci novo) — veći zahvat, planirati zasebno. Ostalo otvoreno: prag `isFiller` (visina ≤ 0.5 m), eventualno uspravljanje polegnutih.
