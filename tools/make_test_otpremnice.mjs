...
// --- definicija otpremnica (EAN + količina, po kupcu) ---
const DOCS = [
  {
    file: 'Otpremnica-TEST-1.xls', doc: 3001, datum: '15.07.2026',
    customers: [
      { name: 'Bijela Tehnika Split d.o.o.', items: [
        ['5949494012852', 4], // perilica 76kg
        ['5949494041470', 1], // škrinja 45kg
        ['5949494040848', 2], // mikrovalna 17.6kg
      ]},
      { name: 'Elektro Marić Rijeka', items: [
        ['5949494019363', 3], // side-by-side 88kg
        ['5949494007445', 2], // perilica 74kg
        ['5949088504770', 2], // napa 18kg
      ]},
      { name: 'Dom Plus Zadar', items: [
  ...
  {
    file: 'Otpremnica-TEST-2.xls', doc: 3002, datum: '15.07.2026',
    customers: [
      { name: 'Tehno Centar Osijek', items: [
        ['5949494019363', 2],
        ['5949494012852', 2],
        ['5949494040848', 1],
      ]},
      { name: 'Kućanski Aparati Varaždin', items: [
        ['5949494044051', 2],
        ['5949494041470', 1],
        ['5949088504770', 3],
        ['5949494007445', 1],
  ...
  {
    file: 'Otpremnica-TEST-3.xls', doc: 3003, datum: '15.07.2026',
    customers: [
      { name: 'Frigo Servis Pula', items: [
        ['5949494029904', 3], // najveći side-by-side 116kg
        ['5949494039774', 2], // perilica 78kg
      ]},
    ],
  },
]
...
const allEans = DOCS.flatMap((d) => d.customers.flatMap((c) => c.items.map((i) => i[0])))
const cat = await fetchCatalog(allEans)

let missing = 0
for (const d of DOCS) {
  let total = 0, lines = 0, units = 0
  for (const c of d.customers) for (const [ean, qty] of c.items) {
    const art = cat.get(ean)
  ...
  console.log(`✅ ${d.file}: ${d.customers.length} kupca, ${lines} stavki, ${units} kom, ${total.toFixed(1)} kg${flag}`)
}
